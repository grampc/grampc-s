...
namespace grampc
{
    ...
    {      
        ...
        for(typeInt k = 1; k < dimX_ + 1; ++k)
        {
            ...
            {
                dcov_dpointsX_vec_(i + k * dimX_) = (diffY_.col(k-1).transpose() * vec(Eigen::seqN(i, dimY_, dimX_))).value() / (4.0 * stepSize_ * stepSize_);
    ...
    const Vector& StirlingInterpolationFirstOrder::dcov_dpointsY_vec(MatrixConstRef pointsX, VectorConstRef vec)
    {
        ...
        for(typeInt k = 1; k < dimX_ + 1; ++k)
        {
            for(typeInt i = 0; i < dimY_; ++i)
            {
                dcov_dpointsY_vec_(i + k * dimY_) = (diffX_.col(k-1).transpose() * vec.segment(i * dimX_, dimX_)).value() / (4.0 * stepSize_ * stepSize_);
